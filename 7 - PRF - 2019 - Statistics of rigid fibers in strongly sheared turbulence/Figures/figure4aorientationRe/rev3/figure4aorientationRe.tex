...
\begin{tikzpicture}
  ...
  \begin{axis}[
    ...
    legend style={at={(axis cs: -85, -0.01)}, anchor=south west},
...
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{1.7e5}$}
  ...
  \path (axis cs: 0,   0.47) coordinate[rec0];  
  \path (axis cs: 45,  0.47) coordinate[rec45];  
  \path (axis cs: -45, 0.47) coordinate[rec45r];  
  \path (axis cs: -90, 0.47) coordinate[rec90];  
  \path (axis cs: 90,  0.47) coordinate[rec90];  
