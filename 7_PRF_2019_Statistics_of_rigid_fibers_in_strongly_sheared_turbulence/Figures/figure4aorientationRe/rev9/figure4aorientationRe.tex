% arara: pdflatex

\documentclass[tikz,border=5pt]{standalone}
\usepackage{pgfplots}
\usepackage{amsmath}
\usepackage{siunitx}
\usepackage{xcolor}

\usepgfplotslibrary{units}

% original colors
\definecolor{color1}{RGB}{166,118,29}
\definecolor{color2}{RGB}{217,95,2}
\definecolor{color3}{RGB}{231,41,138}
\definecolor{color4}{RGB}{117,112,179}
\definecolor{color5}{RGB}{27,158,119}
\definecolor{color6}{RGB}{102,166,30}
\definecolor{color7}{RGB}{230,171,2}

% darkend first colors
\definecolor{color1a}{RGB}{116,68,0}
\definecolor{color3a}{RGB}{181,0,88}
\definecolor{color4a}{RGB}{67,62,119}
\definecolor{color6a}{RGB}{52,116,0}

% lightend second colors
\definecolor{color1b}{RGB}{216,168,79}
\definecolor{color3b}{RGB}{255,91,188}
\definecolor{color4b}{RGB}{167,162,229}
\definecolor{color6b}{RGB}{152,216,80}

\definecolor{seq1}{RGB}{123,204,196}
\definecolor{seq2}{RGB}{67,162,202}
\definecolor{seq3}{RGB}{8,104,172}

\definecolor{seqa}{RGB}{253,204,138}
\definecolor{seqb}{RGB}{252,141,89}
\definecolor{seqc}{RGB}{215,48,31}

\definecolor{seqA}{RGB}{194,230,153}
\definecolor{seqB}{RGB}{120,198,121}
\definecolor{seqC}{RGB}{35,132,67}

% color symbols
\pgfkeys{
 /pgfplots/colorboxtitle/.style={%
  /pgfplots/legend image code/.code={%
      \node [rotate=90, anchor=south] at (2.0mm , 0mm) {\normalsize
          $\alpha=\SI{0.025}{\percent}$};
      \node [rotate=90, anchor=south] at (5.00mm , 0mm) {\normalsize 
          $\alpha=\SI{0.050}{\percent}$};
      \node [rotate=90, anchor=south] at (8.00mm , 0mm) {\normalsize 
          $\alpha=\SI{0.100}{\percent}$};
  },%
 },
}
\pgfkeys{
 /pgfplots/colorboxdark/.style={%
  /pgfplots/legend image code/.code={%
      \fill[seqc] (-1.00mm,-1.00mm) rectangle (1.00mm,1.00mm);
      \fill[seq3] (2.00mm,-1.00mm) rectangle (4.00mm,1.00mm);
      \fill[seqC] (5.00mm,-1.00mm) rectangle (7.00mm,1.00mm);
  },%
 },
}
\pgfkeys{
 /pgfplots/colorboxmiddle/.style={%
  /pgfplots/legend image code/.code={%
      \fill[seqb] (-1.00mm,-1.00mm) rectangle (1.00mm,1.00mm);
      \fill[seq2] (2.00mm,-1.00mm) rectangle (4.00mm,1.00mm);
      \fill[seqB] (5.00mm,-1.00mm) rectangle (7.00mm,1.00mm);
  },%
 },
}
\pgfkeys{
 /pgfplots/colorboxlight/.style={%
  /pgfplots/legend image code/.code={%
      \fill[seqa] (-1.00mm,-1.00mm) rectangle (1.00mm,1.00mm);
      \fill[seq1] (2.00mm,-1.00mm) rectangle (4.00mm,1.00mm);
      \fill[seqA] (5.00mm,-1.00mm) rectangle (7.00mm,1.00mm);
  },%
 },
}

\newcommand\ymax{8.5}
\newcommand\fibymax{0.97}

\begin{document}
\begin{tikzpicture}
  [
    rec0/.style={shade,
                rectangle,
                minimum width=6mm,
                minimum height=2.00mm,
                inner sep=0pt,
                outer sep=0pt,
                top color=gray!60!white,
                bottom color=gray!30!white,
                draw=black, 
                very thin},
    rec45/.style={shade,
                rectangle,
                rotate=45,
                minimum width=6mm,
                minimum height=2mm,
                inner sep=0pt,
                outer sep=0pt,
                top color=gray!60!white,
                bottom color=gray!30!white,
                draw=black, 
                very thin},
    rec45r/.style={shade,
                rectangle,
                rotate=-45,
                minimum width=6mm,
                minimum height=2mm,
                inner sep=0pt,
                outer sep=0pt,
                top color=gray!60!white,
                bottom color=gray!30!white,
                draw=black, 
                very thin},
    rec90/.style={shade,
                rectangle,
                rotate=90,
                minimum width=6mm,
                minimum height=2mm,
                inner sep=0pt,
                outer sep=0pt,
                top color=gray!60!white,
                bottom color=gray!30!white,
                draw=black, 
                very thin}
  ]
% width is 8.64cm
\newcommand{\figwidth}{8.64cm}
\newcommand{\figheight}{5.12cm} % Golden ratio

\newcommand{\goldenratio}{1.618033988749895} 
\newcommand{\axiswidth}{7.4cm} 
% \newcommand{\axisheight}{\axiswidth / \goldenratio - 0.32cm} 
\newcommand{\axisheight}{\axiswidth / \goldenratio - 0.5cm} 

\draw[use as bounding box, white] (0,0) rectangle (\figwidth,\figheight);

%   \begin{axis}[
%     xshift=1.15cm,
%     yshift=0.67cm,
%     clip mode=individual,
%     xmin=-90, xmax=90,
%     ymin=0,
%     ymax=0.49,
%     enlargelimits=false,
%     axis on top=false,
%     width = \axiswidth,
%     height = \axisheight,
%     scale only axis=true,
%     ylabel near ticks,
%     xtick={-90, -45, 0, 45, 90},
%     xticklabels={\ang{-90}, \ang{-45}, \ang{0}, \ang{45}, \ang{90}},
%     xtick pos=right,
%     % xticklabels={$-\frac{\pi}{2}$,
%     %              $-\frac{\pi}{4}$,
%     %              0,
%     %              $\frac{\pi}{4}$,
%     %              $\frac{\pi}{2}$}, 
%     xtick style={font=\small},
%     ytick style={font=\small},
%     minor ytick={0.1, 0.3},
%     minor xtick={-75, -60, -30, -15, 15, 30, 60, 75},
%     xlabel style={yshift=1.65mm},
%   ]

%  \end{axis}

  \begin{axis}[
    xshift=1.15cm,
    yshift=0.67cm,
    clip mode=individual,
    xmin=-90, xmax=90,
    ymin=0,
    ymax=\ymax,
    enlargelimits=false,
    axis on top=false,
    xlabel=$\theta_p$,
    x unit=\si{\degree},
    ylabel=$\text{PDF}(\theta_p)\cdot 10^3$,
    y unit=\frac{1}{\si{\degree}},
    legend style={draw=none, 
                  font=\normalsize,
                  fill=none,
                  row sep=-0.9mm,
    },
    legend plot pos=left,
    legend style={at={(axis cs: -80, 0.01)}, anchor=south west},
    legend cell align=left,
    %reverse legend,
    width = \axiswidth,
    height = \axisheight,
    scale only axis=true,
    ylabel near ticks,
    xtick={-90, -45, 0, 45, 90},
    % xticklabels={\ang{-90}, \ang{-45}, \ang{0}, \ang{45}, \ang{90}},
    % xticklabels={$-\frac{\pi}{2}$,
    %              $-\frac{\pi}{4}$,
    %              0,
    %              $\frac{\pi}{4}$,
    %              $\frac{\pi}{2}$}, 
    xtick style={font=\small},
    ytick style={font=\small},
    minor ytick={1, 3, 5, 7},
    minor xtick={-75, -60, -30, -15, 15, 30, 60, 75},
    xlabel style={yshift=1.65mm},
  ]

  % legend
  \addlegendimage{colorboxtitle}
  \addlegendentryexpanded{ }
  \addlegendimage{colorboxdark}
  \addlegendentryexpanded{$\text{Re}_i=\num{2.5e5}$}
  \addlegendimage{colorboxmiddle}
  \addlegendentryexpanded{$\text{Re}_i=\num{1.7e5}$}
  \addlegendimage{colorboxlight}
  \addlegendentryexpanded{$\text{Re}_i=\num{8.3e4}$}
  
  \addplot[
    forget plot,
    mark=none,
    black,
    thick,
    dashed] coordinates {(0,3.8) (58,3.8)};
    % dashed] coordinates {(0,0.22) (58,0.22)};

  \addplot[
    forget plot,
    mark=none,
    black,
    thick,
    dashed] coordinates {(-90,7.5) (20,7.5)};
    % dashed] coordinates {(-90,0.42) (20,0.42)};

  \addplot+[
    color=seq1,
    mark=none,
    very thick,
    line join=round,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.005_8.27e+04.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.05}{\percent}$, Re$_i = \num{8.3e4}$}

  \addplot+[
    color=seq2,
    mark=none,
    very thick,
    line join=round,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.005_1.65e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.05}{\percent}$, Re$_i = \num{1.7e5}$}

  \addplot+[
    color=seq3,
    mark=none,
    very thick,
    line join=round,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.005_2.48e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.05}{\percent}$, Re$_i = \num{2.5e5}$}

  \addplot+[
    color=seqa,
    mark=none,
    very thick,
    line join=round,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.0025_8.27e+04.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{8.3e4}$}

  \addplot+[
    color=seqb,
    mark=none,
    very thick,
    line join=round,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.0025_1.65e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{1.7e5}$}

  \addplot+[
    color=seqc,
    mark=none,
    very thick,
    line join=round,
    solid,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.0025_2.48e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{2.5e5}$}

  \addplot+[
    color=seqA,
    mark=none,
    very thick,
    line join=round,
    solid,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
] {orientation_0.01_8.27e+04.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{8.3e4}$}

  \addplot+[
    color=seqB,
    mark=none,
    very thick,
    line join=round,
    solid,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
    ] {orientation_0.01_1.65e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{1.7e5}$}

  \addplot+[
    color=seqC,
    mark=none,
    very thick,
    line join=round,
    solid,
   ] table [
    x = bins,
    %y = hist,
    y expr = \thisrow{hist} * 1000,
    % y expr = \thisrow{hist} * (180/3.14159265359),
    col sep=comma,
    ] {orientation_0.01_2.48e+05.csv};
%\addlegendentryexpanded{$\alpha=\SI{0.025}{\percent}$, Re$_i = \num{1.7e5}$}

  % Plot reference particles
\path (axis cs: 0,   \fibymax * \ymax) coordinate[rec0];  
  \path (axis cs: 45,  \fibymax * \ymax) coordinate[rec45];  
  \path (axis cs: -45, \fibymax * \ymax) coordinate[rec45r];  
  \path (axis cs: -90, \fibymax * \ymax) coordinate[rec90];  
  \path (axis cs: 90,  \fibymax * \ymax) coordinate[rec90];  

  \draw[black, <->, very thick, >=stealth] (axis cs: 10, 3.9) --
  node[black, right]  {$\sim 40\%$} (axis cs:10, 7.4);

 \end{axis}


\node at (-0.9, 3.2) {(a)};

\end{tikzpicture} 
\end{document}
